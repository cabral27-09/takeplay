...
const Filmes = () => {
  ...
  // Filter movies by content_type = 'filme' and optionally by genre
  const filteredMovies = useMemo(() => {
    let result = movies.filter(m => m.content_type === 'filme');
  ...
  return (
    <Layout>
      <title>Filmes - TieFlix</title>
      <meta name="description" content="Explore nossa coleção de filmes independentes. Drama, comédia, documentários e muito mais." />
        ...
        >
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl md:text-4xl font-bold">
              {selectedGenreName ? `Filmes: ${selectedGenreName}` : 'Filmes'}
            </h1>
          </div>
          <p className="text-muted-foreground">
            Explore nossa coleção de filmes independentes
          </p>
        </motion.div>
        ...
        {isLoadingMovies ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-muted-foreground text-lg">
              {generoParam 
                ? `Nenhum filme encontrado para o gênero "${selectedGenreName}"`
